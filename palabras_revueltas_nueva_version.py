...
def mesclar_palabra(ruta_tema_elegido, ruta_de_jugador,promedio, maximo):
    ...
    while True:
        if contador != 4:
            ...
            if not dificultad:
                ...
                while contador:
                    palabra = choice(tema_elegido).upper()
                    if len(palabra) <= int(maximo) and len(palabra) >= int(promedio):
                        x = list(palabra)
                        shuffle(x)
                        palabra_elegida = palabra
                        contador +=1
...
while True:
    if contador != 3:
        palabra_elegida, x = mesclar_palabra(ruta_tema_elegido, ruta_de_jugador,medio, maximo)
